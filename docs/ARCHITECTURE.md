...
│   ├── follow_target_publisher.py#   跟随控制器（PID / MPPI 双模式）
│   ├── lock_manager_node.py      #   目标锁定管理
│   ├── gps_waypoint_follower.py  #   GPS-PID 路点导航（Dijkstra 路径规划）
│   ├── lidar_emergency_stop.py   #   LiDAR 前方障碍物安全门控
...
│   │   ├── follow.launch.py      #     跟随层（follow_target_publisher）
│   │   ├── navigation.launch.py  #     导航层（GPS导航、急停、召唤、MQTT）
│   │   └── ...                   #     其他辅助 launch
│   ├── config/                   #     参数配置文件
│   └── urdf/                     #     机器人模型描述
│
...
│  /gps/fix (NavSatFix)  ──►   Dijkstra + GPS-PID                           │
│  /heading_deg (Float32) ──►   航向控制                                      │
│  /scan (LaserScan)      ──►                                                │
│                                    │                                        │
│                                    ▼                                        │
│                          /cmd_vel_nav (Twist)                               │
...
NAVIGATION 模式:
  [gps_waypoint_follower] ──► /cmd_vel_nav ──► [lidar_emergency_stop] ──► /cmd_vel
...
| `/cmd_vel` | `geometry_msgs/Twist` | follow_target_publisher / lidar_emergency_stop / mode_manager | 底盘 STM32 | 10-20 Hz |
| `/cmd_vel_nav` | `geometry_msgs/Twist` | gps_waypoint_follower | lidar_emergency_stop | 10 Hz |
...
| `/nav_blocked` | `std_msgs/String` | lidar_emergency_stop | gps_waypoint_follower | 事件 |
...
| `/scan` | `sensor_msgs/LaserScan` | N10P 驱动 | lidar_emergency_stop | 10 Hz |
...
| `/emergency_stop_state` | `std_msgs/String` | lidar_emergency_stop | test_logger | 20 Hz |
...
| `gps_waypoint_follower` | gps_waypoint_follower.py | GPS-PID 路点导航器，Dijkstra 最短路径规划，方位角 PID 航向控制，输出 /cmd_vel_nav |
| `lidar_emergency_stop` | lidar_emergency_stop.py | 门控 /cmd_vel_nav → /cmd_vel，前方 ±30° 扇形区域障碍物检测：透传/减速/停车三级策略 |
